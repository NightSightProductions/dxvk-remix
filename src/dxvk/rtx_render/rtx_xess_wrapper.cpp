...
namespace dxvk {
  ...
  bool XeSSContext::initialize() {
    ...
    XessError err = xessDeviceQueryCapability(&deviceCapability);
  ...
  XessQuality XeSSContext::getCurrentQualitySetting() const {
    return convertQualitySetting();
  }

  XessQuality XeSSContext::convertQualitySetting() const {
    ...
    switch (qualityLevel) {
      case DlssPreset::UltraPerformance:
        return XESS_QUALITY_PERFORMANCE;
      case DlssPreset::Performance:
        return XESS_QUALITY_BALANCED;
      case DlssPreset::Balanced:
        return XESS_QUALITY_QUALITY;
      case DlssPreset::Quality:
      default:
        return XESS_QUALITY_ULTRA_QUALITY;
    }
  }
  ...
  bool XeSSContext::createContext(uint32_t renderWidth, uint32_t renderHeight,
    ...
    XessError err = xessSrCreateContext(&createInfo, &m_xessContext);
  ...
  bool XeSSContext::dispatch(
    ...
    // Dispatch XeSS
    XessError err = xessSrDispatch(m_xessContext, &dispatchInfo);
